...
Network_analysis <- function(
  ...
  get_protein_idMARTfull <- function(names, mart, Id_mapp_data, flag_biomart) {
    ...
    if(length(names) > 0) {
      ...
      if(flag_biomart) {
        ...
        aux2 <- data.frame(
          entrezgene_id = Id_mapp_data$NCBI_ChEBI[which(Id_mapp_data$Uniprot_ChEBI %in% names)],
  ...
  centrality_extraxtion <- function (
    aux,
    save_name,
    i,
    mart,
    phenotype_comparison,
    Transcriptomics_DTU_total,
    flag,
    multi_omics_data,
    enhancement_flag,
    ...
    if ((length(which(is.na(aux))) == length(aux)) == FALSE) {
      ...
      if(max(lengths(aux_decomposed)) > 10) {
        ...
        if(length(which(as.numeric(aux_bck[, 3]) == 1)) < 0.9 * nrow(aux_bck)) {
          ...
          weights_val <- rep(1, length(aux_decomposed_edges[, 1]))

        }

        aux_uniprot <- get_uniprot_idMART(
          aux_decomposed_edges,
          mart,
          Id_mapp_data,
          flag_biomart,
          weights_val
        ...
        aux_symbol_removes <- get_symbol_idMART(
          aux_edges_removes,
          mart,
          Id_mapp_data,
          flag_biomart,
          rep(1, length(aux_edges_removes[,1]))
        )
        ...
        fsx <- function(k) {
          if ((k + nx_per_worker - 1) <= length_cent_m) {
            ...
            amx <- calculate_centralities(aux_decomposed, include = cent_m[k:length_cent_m])

          }

          if(length(which(lengths(amx) == 0)) > 0) {

            amx <- amx[-which(lengths(amx) == 0)]
        ...
        if(length(which(lengths(centralities_values_CINNA) == 0)) > 0) {
          ...
          centralities_values_CINNA1 <- centralities_values_CINNA
        ...
        if(flag_biomart) {
          ...
          axx <- getBM(
            filters = "entrezgene_id",
            attributes = c("entrezgene_id", "external_gene_name"),
            values = as.character(rownames(centralities_values_CINNA1)),
            mart = mart,
            useCache = FALSE
          )
          ...
          axx <- data.frame(
            entrezgene_id = Id_mapp_data$NCBI_ChEBI[which(Id_mapp_data$NCBI_ChEBI %in% rownames(centralities_values_CINNA1))],
        ...
        if (enhancement_flag) {
          ...
          vv1 <- data.frame(
            matrix(
              data = 0,
              nrow = nrow(centralities_values_CINNA1),
              ncol = length(multi_omics_data)
            )
          )
          ...
          xlsx::write.xlsx(
            unique(
              as.character(
                rbind(aux_bck[, 1], aux_bck[, 2])
              )
            ),
            file = save_name1,
            sheetName = phenotype_comparison[i],
            col.names = FALSE,
            row.names = FALSE,
            append = TRUE
          )
        ...
        save_name1 <- paste(
          Results_dir,
          '/PPINetworks_centralities_values_CINNA_',
          save_name,
          '.xlsx',
          sep = ''
        )
  ...
  add_splicing_inforamtion <- function(
    i,
    symmetry_value,
    data_DTU,
    file_DEA_edges,
    ...
    if(i < symmetry_value + 1) {
      factor <- i
    } else {
      factor <- i - symmetry_value
    }
  ...
  add_edge_weights <- function(
    ...
    file_data <- openxlsx::read.xlsx(
      xlsxFile = file_DEA_names[file_index],
      sheet = phenotype_comparison[sheet_index],
      colNames = TRUE
    )
    ...
    if(nrow(aux) > 0) {
      ...
      if(length(ind_col) > 0 && length(unique(file_data[, ind_col])) > 2) {
        ...
        if ((length(which(is.na(aux))) == length(aux)) == FALSE) {
          ...
          if (length(ind_c) > 0) {
            file_data <- file_data[-ind_c, ]
          }

          file_data <- file_data %>%
            filter(grepl("^[0-9]+\\.?[0-9]*$", .[[ind_col]]))
          
          map_ids <- get_protein_idMARTfull(
            file_data$UniProt_ChEBI,
            mart,
            Id_mapp_data,
            flag_biomart
          )
  ...
  for (j in 1:length(file_DEA_names)) {
    for (i in 1:length(phenotype_comparison)) {
      ...
      if(length(aux) > 0) {
        ...
        if(any(sapply(aux$UniProt_ChEBI, function(x) grepl("[a-df-zA-DF-Z]", x))) == FALSE) {
          id_small_molec[j] <- 1
        }

        file_DEA[[j]][[i]] <- as.character(
          unique(
            get_protein_idMART(
              aux$UniProt_ChEBI,
              mart,
  ...
  for (i in 1:length(phenotype_comparison)) {
    ...
    for (j in 1:length(file_DEA_names)) {
      ...
      if(splicing_file_index > 0) {
        ...
        centrality_extraxtion(
          aux,
          file_DEA_names_short[j],
          i,
          mart,
          phenotype_comparison,
          file_DEA_total[[splicing_file_index]],
          0,
          c(),
          0,
          file_DEA_names_short,
    ...
    if(splicing_file_index > 0) {
      ...
      aux_total <- add_splicing_inforamtion(
        i,
        symmetry_value,
        file_DEA_edges[[splicing_file_index]],
        file_DEA_edges,
        aux_total,
        splicing_file_index,
        file_DEA_names,
