...
run_token <- function(token,working_dir,BioGRID_data_file,STRING_data_file,IntAct_data_file,file_DEA_names,
                      ...
                      files_edges_path,centralities_file,TF_Database,file_extension,Kinome_database,BioMart_Dataset,Client_email,weight_penalty){
  ...
  Integrative_Network_analysis(working_dir,BioGRID_data_file,STRING_data_file,IntAct_data_file,file_DEA_names,
                               phenotype_names,phenotype_comparison,splicing_file_name,
                               Use_precompiled_database,LookUp_table_file,Results_index,BioMart_Dataset,weight_penalty)
  ...
  if(length(Client_email)!=0){
    system(paste0("echo ", "Your analysis associated with the results folder:",Results_index, " is done. Alternatively, you can download your results from the following address: omics-oracle.com/Downloads/",Results_index,".zip. If something did not went as expected check the log files. Otherwise, contact the authors. | mutt -s ",'"NOODAI analysis results are available" ',Client_email))
  }
}
          ...
          var RedirectSite = function(tabName){window.open('https://omics-oracle.com/');}
                                   ...
                                   tabPanel("",
                                            tabPanel("",
                                                       ...
                                                       conditionalPanel( condition = "output.panelStatus",
                                                                         ...
                                                                         conditionalPanel( condition = "input.Type_of_analysis == 'Run the full pipeline'",
                                                                                           fluidRow(
                                                                                             ...
                                                                                             column(3,span(textInput(width = "100%",inputId="phenotype_comparison", label=tooltip(trigger=list("Conditions contrasts",icon("info-circle")),"Provide the analyzed conditions (phenotypes) contrasts. They should be the names of the xlsx sheets. Format example: CoolvsHot3,mkeyvsCool,Hot3vsCool. Use 'vs' to write the contrasts")))),
                                                                                             column(3,span(fileInput(width = "100%",inputId="file_DEA_names3", label=tooltip(trigger=list("Omics files archive",icon("info-circle")),"Upload an archive containing at least one omics file. Each omics file needs to be an Excel table with the sheets containing a 'UniProt_ChEBI' column with Uniprot or ChEBI Ids in accordance with the documentation")))),
                                                                                             column(2,span(sliderInput(width = "100%",inputId="weight_penalty1", min=0.01, max=1, value=0.1,step = 0.01,ticks = FALSE, label=tooltip(trigger=list("Weight factor",icon("info-circle")),"Select the NetWalk framework restart probablity for weighted networks. This value will be used only if the input tables have a node weight column named 'Weight'")))),
                                                                         ...
                                                                         conditionalPanel( condition = "input.Type_of_analysis == 'Custom algorithms'",   
                                                                                           ...
                                                                                           fluidRow(
                                                                                             ...
                                                                                             column(2,span(textInput(width = "100%",inputId="phenotype_comparison1", label=tooltip(trigger=list("Conditions contrasts",icon("info-circle")),"Provide the analyzed conditions (phenotypes) contrasts. They should be the names of the xlsx sheets. Format example: CoolvsHot3,mkeyvsCool,Hot3vsCool. Use 'vs' to write the contrasts")))),
                                                                                             column(2,span(textInput(width = "100%",inputId="BioMart_Dataset",value = "hsapiens_gene_ensembl", label=tooltip(trigger=list("BioMart dataset",icon("info-circle")),"Provide the BioMart dataset used for the mapping of the protein Ids. For other organisms than the pre-loaded ones, provide the pre-formated interaction table! Check the documentation to see the full list of pre-loaded organisms")))),
                                                                                             ...
                                                                                             column(3,span(selectInput(width = "100%",inputId="Use_precompiled_database",choices=c("No"="0","Yes"="1"),selected ='1', label=tooltip(trigger=list("Use Pre-compiled Interaction file",icon("info-circle")),"Choose if you would like to use a pre-compiled interaction file, formated in accordance with the instructions. A default interaction file is available. For other organisms than the pre-loaded ones, upload a pre-compiled interaction file!"))))
                                                                                           )),
                                                                         ...
                                                                         conditionalPanel( condition = "input.Type_of_analysis == 'Custom algorithms'",
                                                                                           conditionalPanel( condition = "input.Use_precompiled_database == '0'",
                                                                                                             fluidRow(
                                                                                                               ...
                                                                                                               column(2,span(fileInput(width = "100%",inputId="STRING_data_file", label=tooltip(trigger=list("STRING database file",icon("info-circle")),"Upload the STRING database file. Leave empty for default version 11.5")),style="opacity:0.5; input-group{opacity:0.5;}")),
                                                                                                               column(2,span(fileInput(width = "100%",inputId="IntAct_data_file", label=tooltip(trigger=list("IntAct database file",icon("info-circle")),"Upload the IntAct database file. Leave empty for default release 245")),style="opacity:0.5; input-group{opacity:0.5;}")),
                                                                                                               column(2,span(fileInput(width = "100%",inputId="file_DEA_names1", label=tooltip(trigger=list("Omics files archive",icon("info-circle")),"Upload an archive containing at least one omics file. Each omics file needs to be an Excel table with the sheets containing a 'UniProt_ChEBI' column with Uniprot or ChEBI Ids in accordance with the documentation")))),
                                                                                                               column(2,span(sliderInput(width = "100%",inputId="weight_penalty", min=0.01, max=1, value=0.1,step = 0.01,ticks = FALSE, label=tooltip(trigger=list("Weight factor",icon("info-circle")),"Select the NetWalk framework restart probablity for weighted networks. This value will be used only if the input tables have a node weight column named 'Weight'"))))
                                                                                                             )),
                                                                                           conditionalPanel( condition = "input.Use_precompiled_database == '1'",
                                                                                                             fluidRow(
                                                                                                               ...
                                                                                                               column(3,span(fileInput(width = "100%",inputId="file_DEA_names2", label=tooltip(trigger=list("Omics files archive",icon("info-circle")),"Upload an archive containing at least one omics file. Each omics file needs to be an Excel table with the sheets containing a 'UniProt_ChEBI' column with Uniprot or ChEBI Ids in accordance with the documentation")))),
                                                                                                               column(2,span(sliderInput(width = "100%",inputId="weight_penalty", min=0.01, max=1, value=0.1,step = 0.01,ticks = FALSE, label=tooltip(trigger=list("Weight factor",icon("info-circle")),"Select the NetWalk framework restart probablity for weighted networks. This value will be used only if the input tables have a node weight column named 'Weight'"))))
                                                                                                             ))
                                                                         ),
                                                                         fluidRow(
                                                                         ...
                                                                         conditionalPanel( condition = "input.Type_of_analysis == 'Run the full pipeline'",
                                                                                           fluidRow(
                                                                                             ...
                                                                                             column(2,span(selectInput(width = "100%",inputId="CPDB_databasesFULL",choices = c("Reactome","Wikipathways", "BioCarta", "PID", "NetPath", "HumanCyc", "INOH","SMPDB"),selected="Reactome",multiple=TRUE, label=tooltip(trigger=list("Pathways databases",icon("info-circle")),"Provide the names of the databases against which the enrichment will be performed. Example: Reactome, Wikipathways")))),
                                                                                             column(2,span(textInput(width = "100%",inputId="BioMart_Dataset1",value = "hsapiens_gene_ensembl", label=tooltip(trigger=list("BioMart dataset",icon("info-circle")),"Provide the BioMart dataset used for the mapping of the protein Ids. For other organisms than the pre-loaded ones, provide the pre-formated interaction table! The available biomartID is only for humans. Alternatives: mmusculus_gene_ensembl, rnorvegicus_gene_ensembl, btaurus_gene_ensembl, dmelanogaster_gene_ensembl, drerio_gene_ensembl. See the documentation for the full list")))),
                                                                         ...
                                                                         conditionalPanel( condition = "input.Type_of_analysis == 'Run the full pipeline'",
                                                                                           ...
                                                                                           br(),
                                                                                           fluidRow(
                                                                                             column(offset=1,10, tags$img(src = "Ribbon_pipeline.jpg",  style = "width: 100%; max-width: 2400px; height: 100vh; height: auto; display: block; margin: 0 auto;")))
                                                                                             ...
                                                                                             conditionalPanel( condition = "input.Type_of_analysis == 'Custom algorithms'",
                                                                                                               ...
                                                                                                               fluidRow(
                                                                                                                 column(offset=1,3,span(selectInput(width = "100%",inputId="CPDB_databases",choices = c("Reactome","Wikipathways", "BioCarta", "PID", "NetPath", "HumanCyc", "INOH","SMPDB"),selected="Reactome",multiple=TRUE, label=tooltip(trigger=list("CPDB databases",icon("info-circle")),"Provide the names of the databases against which the enrichment is to be performed. Example: Reactome, Wikipathways")))),
                                                                         ...
                                                                         conditionalPanel( condition = "output.showRes",
                                                                                           fluidRow(
                                                                                             ...
                                                                                             column(12,align="center",span(htmlOutput("verb"),style="font-weight:bold;")),
                                                                         ...
                                                                         conditionalPanel( condition = "output.showResCentralities",
                                                                                           fluidRow(
                                                                                             column(12,align="center",htmlOutput("CentralitiesOut1")),
                                                                         ...
                                                                         conditionalPanel( condition = "output.showResMONET",
                                                                                           fluidRow(
                                                                                             column(12,align="center",htmlOutput("MONETOut1")),
                                                                         ...
                                                                         conditionalPanel( condition = "output.showResPathways",
                                                                                           fluidRow(
                                                                                             column(12,align="center",htmlOutput("PathwaysOut1")),
                                                                         ...
                                                                         conditionalPanel( condition = "output.showResImages",
                                                                                           fluidRow(
                                                                                             column(12,align="center",htmlOutput("ImagesOut1")),
                                                     ...
                                                     )),fluidPage( tags$footer("The analysis pipeline is provided for use 'as is' and 'as available' under GNU GPL v3, see <https://www.gnu.org/licenses/>. The authors make no representations or warranties of any kind, expressed or implied, regarding the performance, reliability, or suitability of the analysis.\n By using this website, you agree that the authors shall not be responsible for any data loss, corruption, or unauthorized access to your data. It is your responsibility to implement appropriate backup and security measures to safeguard your data. NOODAI Copyright (C) 2025, Empa, Tiberiu Totu.", class = "footer")
                        ...
                        server <- function(input, output, session) {
                          ...
                          output$text_output_version <- renderText({"version 1.1.0"})
                                    ...
                                    if(analyses_flag){
                                      ...
                                      jobs[[token]] <<- callr::r_bg(Integrative_Network_analysis, supervise = FALSE, stdout = paste0(working_dir,"/Results_",Results_index(),"/out_Centralities.txt"), package=TRUE, stderr = paste0(working_dir,"/Results_",Results_index(),"/log_Centralities.txt"),
                                                                    args = list(working_dir=working_dir,BioGRID_data_file=BioGRID_data_fileVal,STRING_data_file=STRING_data_fileVal,
                                                                                ...
                                                                                LookUp_table_file=LookUp_table_fileVal,Results_index=Results_index(),BioMart_Dataset=BioMart_Dataset,weight_penalty=weight_penalty))
                                      ...
                                      for (j in 1:length(file_DEA_names)){
                                        ...
                                        for (i in 1:length(phenotype_comparison)){
                                          ...
                                          if(length(aux)>0){
                                            ...
                                            # aux1 <- as.character(unique(get_protein_idMART(aux$UniProt_ChEBI,mart)))
                                            aux1 <- aux$UniProt_ChEBI[which(aux$UniProt_ChEBI %in% uniprot_ids_maps$UniProt_ChEBI)]
                                            if(length(aux1)<(length(aux$UniProt_ChEBI)/3)){
                                              showVal(FALSE)
                                              showErrorFormatMessage(TRUE)
                                    ...
                                    if(analyses_flag){
                                      jobs[[token]] <<- callr::r_bg(run_token, supervise = FALSE, stdout = paste0(working_dir,"/Results_",Results_index(),"/out.txt"), package=TRUE, stderr = paste0(working_dir,"/Results_",Results_index(),"/log.txt"),
                                                                    args = list(token=token,working_dir=working_dir,BioGRID_data_file=BioGRID_data_fileVal,STRING_data_file=STRING_data_fileVal,IntAct_data_file=IntAct_data_fileVal,file_DEA_names=file_DEA_names,
                                                                                ...
                                                                                files_edges_path=files_edges_path(),centralities_file=centralities_file(),TF_Database=TF_Database(),file_extension=file_extension(),Kinome_database=Kinome_database(),BioMart_Dataset=BioMart_Dataset,Client_email=Client_email(),weight_penalty=weight_penalty))
                          ...
                          output$verb <- renderUI({ HTML("Your results folder is: ", Results_index(),"<br>Additionally you can download the results from: omics-oracle.com/Downloads/",Results_index(),".zip") })
                          ...
                          output$CentralitiesOut1 <- renderUI({ HTML("Your results folder is: ", Results_index(),"<br>Additionally you can download the results from: omics-oracle.com/Downloads/",Results_index(),".zip") })
                          ...
                          output$MONETOut1 <- renderUI({ HTML("Your results folder is: ", Results_index(),"<br>Additionally you can download the results from: omics-oracle.com/Downloads/",Results_index(),".zip") })
                          ...
                          output$PathwaysOut1 <- renderUI({ HTML("Your results folder is: ", Results_index(),"<br>Additionally you can download the results from: omics-oracle.com/Downloads/",Results_index(),".zip") })
                          ...
                          output$ImagesOut1 <- renderUI({ HTML("Your results folder is: ", Results_index(),"<br>Additionally you can download the results from: omics-oracle.com/Downloads/",Results_index(),".zip") })
                          ...
                          output$showErrorSample1 <- renderText({ paste0("The provided samples names may be incorrect. Please use the same names that are included in the contrasts and in the Excel sheets.\n",special_error_messageSample()) })
                          ...
                          output$showErrorContrasts1 <- renderText({ paste0("The provided contrasts may be incorrect. Please format the contrasts to include the samples names and be the same as the Excel sheets.\n",special_error_messageContrast()) })
